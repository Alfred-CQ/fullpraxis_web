...
export function CardDemo({ name, monthly_discount, enrollment_discount, description, className, ...props }: DiscountCardProps) {
    return (
        <Card className={cn('w-[380px]', className)} {...props}>
            <CardHeader>
                <CardTitle>{name}</CardTitle>
                <CardDescription>{description || 'Descuentos disponibles'}</CardDescription>
            </CardHeader>
